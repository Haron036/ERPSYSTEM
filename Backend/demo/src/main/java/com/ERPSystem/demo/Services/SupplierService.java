...
public class SupplierService {
    ...
    public SupplierDto.Response findById(Long id) {
        return map(repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Supplier not found: " + id)));
    }
    ...
    public SupplierDto.Response update(Long id, SupplierDto.Request req) {
        ...
        s.setCountry(req.getCountry()); s.setRating(req.getRating()); s.setLeadTime(req.getLeadTime());
        return map(repo.save(s));
    }

    public void delete(Long id) {
        if (!repo.existsById(id)) throw new ResourceNotFoundException("Supplier not found: " + id);
        repo.deleteById(id);
    }
    ...
    private SupplierDto.Response map(Supplier s) {
        return SupplierDto.Response.builder()
                ...
                .contactPerson(s.getContactPerson()).country(s.getCountry())
                .rating(s.getRating()).leadTime(s.getLeadTime()).build();
    }
}
