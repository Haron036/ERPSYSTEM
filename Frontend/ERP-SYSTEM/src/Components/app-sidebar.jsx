...
import {
  LayoutDashboard, Users, Boxes, ShoppingCart, Truck,
  Calculator, FolderKanban, HeartHandshake, BarChart3,
  Settings, Factory, Bell, Palette, User,
} from "lucide-react";
import {
...
export function AppSidebar() {
  ...
  function handleSave() {
    // Persist profile display name / theme; real profile update would call PATCH /users/me
    localStorage.setItem("erp_theme", theme);
    setSaved(true);
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <Tabs defaultValue="profile" className="mt-1">
            <TabsList className="w-full">
              ...
              <TabsTrigger value="notifications" className="flex-1 gap-1.5"><Bell      className="h-3.5 w-3.5" />Notifications</TabsTrigger>
            ...
            <TabsContent value="profile" className="mt-4 space-y-4">
              <div className="flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary/10 text-primary text-xl font-semibold select-none">
                  {profile.fullName ? profile.fullName.split(" ").map((p) => p[0]).join("").slice(0, 2).toUpperCase() : "?"}
                </div>
                <div>
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-muted-foreground">Role is managed by your administrator.</p>
              </div>
            ...
            <TabsContent value="notifications" className="mt-4 space-y-1">
              {[
                { key:"lowStock",       label:"Low stock alerts",         desc:"Notify when SKUs fall below reorder point" },
                { key:"overdueInvoice", label:"Overdue invoice alerts",   desc:"Flag invoices past due date"               },
                { key:"newTicket",      label:"New support tickets",      desc:"Alert on incoming CRM tickets"             },
                { key:"approvalNeeded", label:"Approval requests",        desc:"POs and journals awaiting sign-off"        },
                { key:"systemAlerts",   label:"System notifications",     desc:"Maintenance windows and updates"           },
              ].map(({ key, label, desc }) => (
                <div key={key} className="flex items-center justify-between rounded-md px-1 py-2.5 hover:bg-muted/40 transition-colors">
                  <div>
            ...
            <TabsContent value="theme" className="mt-4 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { val:"light",  label:"Light",  bg:"bg-white",        border:"border-2",       preview:"bg-gray-100" },
                  { val:"dark",   label:"Dark",   bg:"bg-zinc-900",     border:"border-2",       preview:"bg-zinc-700" },
                  { val:"system", label:"System", bg:"bg-gradient-to-br from-white to-zinc-800", border:"border",  preview:"bg-gray-400" },
                  ...
                  >
                    <div className={`${bg} rounded-md h-12 w-full mb-1.5 flex flex-col gap-1 p-1.5`}>
                      <div className={`${preview} h-2 rounded w-3/4`} />
                      <div className={`${preview} h-2 rounded w-1/2 opacity-60`} />
                    </div>
